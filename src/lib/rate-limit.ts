...
export async function withinDailyLimit(identifier: string, limit: number): Promise<boolean> {
  ...
  // Service-role only: increment_usage() EXECUTE is revoked from anon/authenticated
  // (it's a SECURITY DEFINER counter anyone could otherwise call to grief quotas).
